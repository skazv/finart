...
protocol BudgetsBoardCollectionlDelegate {
    func delete(cell: AccountCollectionViewCell)
}

////MARK: - CellDelegate
//extension BudgetsBoardCollection: BudgetsBoardCellDelegate {
//    func delete(cell: AccountCollectionViewCell) {
...
class BudgetsBoardCollection: UICollectionView {
    ...
    var budgetsArray: [BudgetViewModel] = []
...
extension BudgetsBoardCollection {
    private func setup() {
        backgroundColor = .secondarySystemBackground//.systemBackground
        layer.cornerRadius = 20
        
        translatesAutoresizingMaskIntoConstraints = false
        register(BudgetsBoardCell.self, forCellWithReuseIdentifier: customCellId)
        dataSource = self
        delegate = self
        isEditing = false
...
extension BudgetsBoardCollection: UICollectionViewDataSource {

    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return budgetsArray.count
    }

    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: customCellId, for: indexPath) as? BudgetsBoardCell else { return UICollectionViewCell() }
        cell.update(name: budgetsArray[indexPath.row].name, count: 0, image: budgetsArray[indexPath.row].icon)
...
extension BudgetsBoardCollection: UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, insetForSectionAt section: Int) -> UIEdgeInsets {
        return UIEdgeInsets(top: giveVerticalSpacing(), left: sectionInserts.left, bottom: giveVerticalSpacing(), right: sectionInserts.right)
    }
...
extension BudgetsBoardCollection: UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if indexPath.row == budgetsArray.count - 1 {
...
extension BudgetsBoardCollection: UIDragInteractionDelegate {
  ...
  //      print("itemsForBeginning")
        if isEditing == false {
            ...
            if let index = indexPathForItem(at: touchedPoint) {
                if index.row != budgetsArray.count - 1 {
                    let image = budgetsArray[index.row].icon
...
extension BudgetsBoardCollection: UIDropInteractionDelegate {
    ...
    func dropInteraction(_ interaction: UIDropInteraction, sessionDidEnd session: UIDropSession) {
        ...
        if let accountDestinationRow = accountDestinationRow {
            if accountDestinationRow == budgetsArray.count - 1 || accountDestinationRow == accountSourceRow {
                return
            }
        }
